...
public class EnrollmentServiceClient : IEnrollmentServiceClient
{
    ...
    public async Task<IEnumerable<object>> GetAllEnrollmentsAsync()
    {
        var accessToken = await GetAuth0OAuth2TokenAsync();
        var request = new HttpRequestMessage(HttpMethod.Get, "api/s2s/enrollments");
        ...
        throw new Exception($"Failed to get all enrollments: {response.StatusCode}");
    }
}