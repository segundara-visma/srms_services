...
public class TutorServiceClient : ITutorServiceClient
{
    private readonly HttpClient _httpClient;
    private readonly HttpClient _auth0HttpClient;
    private readonly Auth0Settings _auth0Settings;

    public TutorServiceClient(HttpClient httpClient, IOptions<Auth0Settings> auth0Settings)
    {
    ...
    {
        var accessToken = await GetAuth0OAuth2TokenAsync();
        var request = new HttpRequestMessage(HttpMethod.Post, $"api/s2s/tutors/{userId}");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
        var response = await _httpClient.SendAsync(request);
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception($"Failed to create tutor: {response.StatusCode}");
        }
    }
    ...
    public async Task AssignCourseToTutorAsync(Guid tutorId, Guid courseId)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new Exception($"Failed to assign course to tutor: {response.StatusCode}");
        }
    }
}